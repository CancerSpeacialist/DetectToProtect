...
export function FileUpload({
  ...
  const getFileTypeLabel = (file) => {
    ...
    } else if (file.name.toLowerCase().includes(".nrrd")) {
      return "NRRD Medical Data";
    }
    return "Medical File";
  };
      ...
      <Card className="bg-blue-50 border-blue-200">
        <CardContent className="p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs text-blue-700">
            ...
            <div>• JPEG (.jpg, .jpeg) - High quality images</div>
            <div>• PNG (.png) - Lossless compression</div>
            <div>• NRRD (.nrrd) - Medical research data</div>
            <div>• Maximum size: {formatFileSize(maxSize)}</div>
            <div>• Ensure good image quality for best results</div>
          </div>
        </CardContent>
      </Card>
    </div>
