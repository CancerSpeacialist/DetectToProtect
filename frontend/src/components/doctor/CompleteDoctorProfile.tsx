...
import { CompleteDoctorProfileData } from "@/lib/types/auth";
...
export default function CompleteDoctorProfile({
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<CompleteDoctorProfileFormData>({
  ...
  const onSubmit = async (data: CompleteDoctorProfileFormData) => {
    try {
      setError("");
      setLoading(true);
      ...
      const profileData: CompleteDoctorProfileData = {
